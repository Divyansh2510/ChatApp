...
const LoginPage = () => {
  ...
  return (
    ...
    justify-center gap-8 sm:justify-evenly max:sm:flex-col backdrop-blur-2xl '>
      ...
      <img src={assets.favicon2} alt="" className='w-[min(3000vw,500px)]'></img>
      ...
      flex-col gap-6 rounded-lg shadow-lg'>
        ...
        {currState === "Sign Up" && !isDataSubmitted && (
          ...
          focus:outline-none' placeholder='Full Name' required></input>
        )}
        ...
        <div className='flex items-center gap-2 text-sm text-gray-500'>
          <input type='checkbox' />
          <p>Agree to terms and policy and privacy use.</p>
        </div>
        
        <div className='flex flex-col gap-2'>
          { currState === 'Sign Up' ? (
            <p className='text-sm  text-gray-600'>Already Having An Account <span
            onClick={()=>{setCurrState("Login"); setIsDataSubmitted(false)}} className='font-medium text-violet-500 cursor-pointer'>
             Login Here </span></p>
          )
          :(
           <p className='text-sm  text-gray-600'>Create an Account <span
            onClick={()=>setCurrState("Sign Up")} className='font-medium text-violet-500 cursor-pointer'>
             Click Here </span></p>
          )
          }
        </div>
